    ...
    public Response receiveToDelete(@PathParam("id") UUID id) {
        ...
        try {
            if (dogsService.deleteDogs(id)) {
                ...
                Log.infof("| REGISTRO DE ID %s DELETADO COM SUCESSO.", id);
                return Response.ok(RESPONSE).build();
            }
